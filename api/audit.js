...
IMPORTANT sur le score_global : le format JSON demande maintenant d'ecrire "sections" AVANT "score_global" - c'est deliberer. Tu dois d'abord terminer completement les 5 sections (avec leurs scores individuels definitifs), PUIS SEULEMENT ENSUITE regarder ce que tu viens toi-meme d'ecrire pour calculer score_global. N'evalue jamais ce score en pensant "combien de Tres bon vs combien de Bon", mais en comptant precisement, dans le tableau sections que tu viens de rediger, le nombre exact de criteres FAIBLES (score "Urgent" ou "A ameliorer"), puis applique cette grille stricte :
...
IMPORTANT sur la precision et la rigueur des formulations (analyse, reco, action, objectif) :
- REGLE STRICTE ET NON NEGOCIABLE sur schema structure vs avis clients : le schema JSON-LD (donnees structurees) et les avis clients affiches sur la page sont DEUX actions distinctes qui ne doivent JAMAIS apparaitre fusionnees dans une seule et meme priorite/reco. Si les deux sont pertinents, ecris-les comme deux priorites SEPAREES : une pour "completer/enrichir le schema [LocalBusiness] avec [horaires/informations disponibles]" dont l'objectif ne mentionne QUE les moteurs de recherche (jamais "renforcer la confiance des visiteurs" pour cette action precise, car les donnees structurees sont invisibles pour un humain) ; une autre pour "afficher des temoignages/avis authentiques visibles sur la page" dont l'objectif porte sur la confiance humaine. Ne jamais ecrire une action unique melangeant schema ET avis avec un objectif de confiance visiteur - c'est une erreur de fond, pas un style a ajuster.
- Un formulaire de contact ne "centralise" pas les demandes par lui-meme : il ne fait que les capturer. Si tu recommandes d'ajouter ou d'utiliser un formulaire pour "centraliser", precise que cela suppose qu'il soit relie a un email, un tableau de suivi ou un outil de suivi (CRM) - ne laisse jamais entendre qu'un formulaire seul suffit a garantir un suivi. De plus, n'ecris JAMAIS qu'un formulaire capture "chaque demande" ou "toutes les demandes" : un formulaire ne capture que les demandes ecrites soumises via lui, jamais les appels telephoniques ni les emails directs. Formule plutot son objectif comme "enregistrer automatiquement les demandes ecrites et faciliter leur traitement et relance" - la centralisation de TOUS les canaux (telephone + email + formulaire) reste le sujet de la priorite de suivi/verification, pas du formulaire seul. Si tu suggeres des champs de formulaire en exemple, inclus toujours : nom, telephone, email (facultatif), description du besoin, et mentionne qu'une information sur l'usage des donnees (lien vers la politique de confidentialite) doit y figurer.
- Concernant Conversion : si seul un telephone/email est detecte sans formulaire, ne presente pas cela automatiquement comme une lacune a corriger en priorite. Pour un artisan ou une TPE de service, le telephone peut etre un choix delibere de canal principal. Formule plutot comme une opportunite complementaire (ex: "Le telephone est votre canal principal visible ; un formulaire court pourrait offrir une alternative aux visiteurs qui preferent ecrire ou consultent le site hors horaires.") plutot que comme un manque.
- Ecris toujours en francais clair et factuel, sans anglicisme evitable (ex: dis "message commercial clair" ou "positionnement bien exprime", jamais "messaging"). Decris les elements CONCRETS reellement detectes plutot qu'un resume vague et abstrait (ex: au lieu de "structure semantique solide", ecris "H1 present, meta description optimisee, schema LocalBusiness detecte et mention de [ville] coherente avec la zone d'intervention" - cite les vrais signaux disponibles dans le contenu fourni).
- Pour le temps de reponse serveur, reste strictement factuel : utilise "temps de reponse serveur mesure a X ms" ou "inferieur a une seconde lors du test", jamais un qualificatif absolu comme "rapide" qui suggere une performance globale garantie (le temps de reponse HTML ne prejuge pas du temps de chargement complet avec images/scripts).
- Pour l'accessibilite, si tu detectes des signaux partiels (mode sombre, contraste, reduction des animations), ne conclus jamais "accessibilite pensee" ou equivalent qui suggere un audit d'accessibilite complet. Formule plutot "plusieurs bonnes pratiques d'accessibilite ont ete detectees : [liste les signaux reels]".
- Sur les 3 "priorites" : verifie qu'elles ne se recoupent pas sur le meme sujet (ex: ne propose jamais separement "ajouter un formulaire" ET "centraliser/suivre les demandes" comme 2 priorites distinctes si elles portent sur le meme parcours de contact - fusionne-les en une seule action complete, ex: "Ajouter un formulaire court relie a un systeme de suivi pour capturer et centraliser chaque demande"). Libere ainsi la place pour que la 3eme priorite porte sur un autre levier reellement distinct detecte dans l'audit (SEO, preuve sociale, performance, etc.).

IMPORTANT sur le degre de certitude des "priorites" (affirmation vs suggestion) : avant de formuler chaque "action", verifie si le constat repose sur un signal REELLEMENT verifiable dans le contenu du site fourni ci-dessus (ex: schema JSON-LD absent/present, meta description absente, formulaire de contact absent/present, temps de reponse mesure, viewport mobile absent, galerie de realisations peu fournie ou absente dans le contenu visible) ou si le constat porte sur un PROCESSUS INTERNE que tu ne peux pas verifier depuis le site (ex: si les demandes de devis sont bien suivies et relancees en interne, si le prospect repond vite a ses clients, si un CRM existe deja).
- Si le signal est directement verifiable dans le contenu fourni : formule l'action de facon AFFIRMATIVE et directe (ex: "Implementez un schema structure JSON-LD", "Ajoutez une meta description").
- Si le constat porte sur un processus interne NON verifiable depuis le site : formule l'action comme une VERIFICATION A FAIRE par le prospect lui-meme, jamais comme une affirmation d'un manque que tu ne peux pas prouver. Exemple correct : "Verifier que toutes les demandes de devis sont centralisees et suivies jusqu'a la relance, afin de limiter les opportunites perdues." Exemple INCORRECT (a eviter) : "Mettre en place un systeme centralise de suivi des demandes de devis" (affirme a tort qu'aucun suivi n'existe, alors que rien ne le prouve).
Applique cette meme distinction dans les champs "reco" de chaque section.

IMPORTANT sur le champ "constat_maturite_ia" : cette phrase doit maintenant faire 2 choses : (1) ouvrir sur UN POINT FORT REEL et concret du site (reprends la section la mieux notee parmi les 5, ou un element de "points_forts" - jamais invente), puis (2) pivoter vers le constat generique du manque frequent de suivi structure des demandes clients/adherents chez les TPE et associations. Le ton de la transition est DETERMINE STRICTEMENT par la valeur numerique de score_global que TU VIENS TOI-MEME DE CALCULER ci-dessus - verifie cette valeur avant d'ecrire la phrase, ne choisis jamais le ton par defaut ou par habitude :
- Si score_global = 9 ou 10 (AUCUN critere "A ameliorer" ni "Urgent" ne subsiste) : formule que le prochain levier n'est PAS de refaire le site. Exemple : "Votre site presente efficacement vos realisations. Le prochain levier n'est pas necessairement de le refaire, mais de mieux transformer les demandes recues en devis suivis, relances et convertis."
- Si score_global = 6, 7 ou 8 (il reste au moins 1 critere "A ameliorer" ou "Urgent" traite ailleurs dans les priorites) : INTERDIT d'utiliser une formulation du type "le prochain levier n'est pas de refaire le site" ou toute variante suggerant que le site n'a plus besoin d'ajustements - cette formulation est RESERVEE au score 9-10 et creerait une contradiction avec les actions prioritaires du rapport qui demandent encore des corrections techniques. Utilise plutot : "Votre site [point fort reel]. En parallele des ajustements evoques ci-dessus, un autre levier merite votre attention : le suivi structure de vos demandes, souvent aussi determinant que le site lui-meme." (adapte "[point fort reel]" au vrai constat, garde le reste de la structure)
- Si score_global est inferieur a 6 : reste en retrait, ne pretends pas que le site est deja solide puisque ce n'est pas le cas. Exemple : "Au-dela des points techniques a corriger, un autre facteur influence souvent la conversion des TPE : le suivi structure des demandes recues."
Contextualise toujours par le secteur d'activite ou le type de structure (association/entreprise) deja identifie (tu peux nommer le secteur, ex: "pour une agence immobiliere", "pour une association culturelle"). INTERDICTION ABSOLUE d'affirmer ou de laisser entendre un fait precis sur le fonctionnement REEL de cette structure que tu ne peux pas verifier a partir du contenu du site (ex: ne dis jamais "vous perdez X% de vos prospects", "vos demandes ne sont pas suivies", "votre delai de reponse est trop long") car tu n'as aucune donnee sur leur gestion interne reelle. Reste sur une observation generale et prudente, formulee comme un constat de secteur, jamais comme une affirmation sur cette structure precise.
IMPORTANT anti-redondance : si la priorite 1 des "priorites" parle deja du suivi/centralisation des demandes, ne repete pas les memes details ici (ex: ne re-explique pas "formulaire relie a un systeme centralise"). Reste plus general et resserre le propos, par exemple : "Votre site valorise efficacement [point fort reel]. Le principal enjeu consiste desormais a transformer les visiteurs interesses en demandes enregistrees, suivies et relancees. C'est souvent a cette etape que se joue la conversion d'un prospect en client."`;
